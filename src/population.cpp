...
bool Population::speciate() {
	...
	for(curorg=hubs.begin();curorg!=hubs.end();++curorg) {
		...
		if (curspecies==species.end()){
			...
			//MJRNODEPOINTERnewspecies.add_Organism((Organism)*curorg);  //Add the current hub
			...
			while((comporg!=0)&&
				(curspecies!=species.end())) {
					...
					if ((((curorg)->gnome).compatibility(&comporg->gnome))<NEAT::compat_threshold) {
						...
						//MJRNODEPOINTERcurspecies->add_Organism(*curorg);
				...
				if (comporg!=0) {
					newspecies=*new Species(++counter);
					species.push_back(newspecies);
					//MJRNODEPOINTERnewspecies.add_Organism(*curorg);  //Add the current hub
					(curorg)->species=newspecies;  //Point hub to its species
				}
...
void Population::reassign_species(Hub *org) {
		...
		//MJRNODEPOINTERcomporg=(Hub*)(curspecies)->first();
		while((curspecies!=(species).end()) && (!found)) 
		{	
			//if (comporg==0) {
				//Keep searching for a matching species
				++curspecies;
				//MJRNODEPOINTERif (curspecies!=(species).end())
					//MJRNODEPOINTERcomporg=(Hub)(*curspecies)->first();
			//}
			/*else*///MJRNODEPOINTER if (((org->gnome).compatibility(comporg->gnome))<NEAT::compat_threshold) {
				//If we found the same species it's already in, return 0
				//MJRNODEPOINTERif ((*curspecies)==org->species) {
					//MJRNODEPOINTERfound=true;				
					//MJRNODEPOINTERbreak;
				//MJRNODEPOINTER}
...
void Population::remove_org(Hub org_to_kill) {
	...
	for(deadorg=hubs.begin();deadorg!=hubs.end();++deadorg) {
		//MJRNODEPOINTERif (*deadorg == org_to_kill )
		    //MJRNODEPOINTER{
